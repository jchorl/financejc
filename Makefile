...
backup:
	docker run -it --rm \
		...
		-v $(PWD)/backup:/backup \
		postgres \
		sh -c 'pg_dump -h financejcdb -d postgres -U "$(POSTGRES_USER)" -Fc > /backup/$$(date +%Y_%m_%d__%H_%M_%S).sql'
...
restore:
	docker run -it --rm \
		...
		-v $(PWD)/backup:/backup \
		postgres \
		pg_restore -h financejcdb -d postgres -U '$(POSTGRES_USER)' --data-only /$(BACKUP_FILE)
